...
class _ChatMessage {
    ...
    private static final int LENGTH_OFFSET = TIMESTAMP_OFFSET
                                             + TIMESTAMP_LENGTH;
...
//    }
    ...
    static class OfArray {
        ...
        private static byte[] requireValid(byte[] array) {
            if (Objects.requireNonNull(array, "array is null").length
                != BYTES) {
                throw new IllegalArgumentException(
        ...
        static long getTimestamp(byte[] array) {
            return getLong(requireValid(array), TIMESTAMP_OFFSET,
                           TIMESTAMP_LENGTH);
        }

        static <R> R getTimestampAsMapped(byte[] array,
        ...
        static void setTimestampAsSupplied(byte[] array,
                                           LongSupplier supplier) {
            Objects.requireNonNull(supplier, "supplier is null");
            setTimestamp(array, supplier.getAsLong());
        }

        static <T> void setTimestampAsMapped(byte[] array,
        ...
        static void printToSystemOut(byte[] array) {
            ...
            var string = TIMESTAMP_FORMATTER.format(temporal) + ' '
                         + getMessage(array);
    ...
    static class OfBuffer {
        ...
        private static ByteBuffer requireValid(ByteBuffer buffer) {
            ...
            if (buffer.arrayOffset() != 0) {
                throw new IllegalArgumentException(
                        "buffer.arrayOffset(" + buffer.arrayOffset()
                        + ") != 0");
            }
        ...
        static <R> R getTimestampAsMapped(ByteBuffer buffer,
                                          LongFunction<R> mapper) {
            Objects.requireNonNull(mapper, "mapper is null");
        ...
        static void setTimestampAsSupplied(ByteBuffer buffer,
                                           LongSupplier supplier) {
            Objects.requireNonNull(supplier, "supplier is null");
            setTimestamp(buffer, supplier.getAsLong());
        }

        static <T> void setTimestampAsMapped(ByteBuffer buffer,
                                             ToLongFunction<? super T> mapper,
                                             T value) {
            Objects.requireNonNull(mapper, "mapper is null");
