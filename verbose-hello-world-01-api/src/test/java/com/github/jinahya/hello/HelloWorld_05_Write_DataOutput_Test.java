...
class HelloWorld_05_Write_DataOutput_Test extends HelloWorldTest {
     ...
     * {@link HelloWorld#set(byte[]) set(array)} method with an array of {@value
     * com.github.jinahya.hello.HelloWorld#BYTES} bytes and writes the array to
    ...
    void write_InvokeSetArrayWriteArrayToData_(@TempDir final File tempDir)
        ...
        helloWorld().write(
                data);                                                    // <2>
        Mockito.verify(helloWorld(), Mockito.times(1)).set(
                arrayCaptor().capture()); // <3>
        final byte[] array =
