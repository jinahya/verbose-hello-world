...
/**
 * A class for testing {@link HelloWorld#write(AsynchronousByteChannel)}
 * method.
 *
...
class HelloWorld_10_Write_AsynchronousByteChannel_Test extends HelloWorldTest {
     ...
     * method invokes {@link HelloWorld#put(ByteBuffer) put(buffer)} method and
     * writes the buffer to the {@code channel}.
     *
    ...
    void write_ReturnChannel_()
        ...
        Mockito.lenient().when(
                        channel.write(ArgumentMatchers.any(ByteBuffer.class)))
                .thenAnswer(i -> {
                    final ByteBuffer buffer = i.getArgument(0,
                                                            ByteBuffer.class);
                    final int written = new Random().nextInt(
