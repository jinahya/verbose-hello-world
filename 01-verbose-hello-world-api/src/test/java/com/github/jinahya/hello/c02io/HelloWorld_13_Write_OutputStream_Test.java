 ...
 * @see HelloWorld_13_Write_OutputStream_Arguments_Test
 */
@DisplayName("write(stream)")
@Slf4j
class HelloWorld_13_Write_OutputStream_Test extends _HelloWorldTest {

    @BeforeEach
    void _beforeEach() {
        doAnswer(i -> i.getArgument(0))                             // <1>
                .when(serviceInstance())                            // <2>
                .set(ArgumentMatchers.<byte[]>argThat(a -> a != null && a.length == BYTES)); // <3>
    }
