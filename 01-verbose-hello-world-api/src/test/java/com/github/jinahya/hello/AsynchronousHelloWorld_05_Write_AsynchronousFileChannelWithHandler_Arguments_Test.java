 ...
 * write(channel, position, handler)} method regarding arguments verification.
 *
 * @author Jin Kwon &lt;onacit_at_gmail.com&gt;
 * @see AsynchronousHelloWorld_05_Write_AsynchronousFileChannelWithHandler_Test
 */
@DisplayName("write(channel, position, handler) arguments")
@Slf4j
class AsynchronousHelloWorld_05_Write_AsynchronousFileChannelWithHandler_Arguments_Test
        extends _AsynchronousHelloWorldTest {

    /**
     * Asserts
     * {@link AsynchronousHelloWorld#write(AsynchronousFileChannel, long, Executor) write(channel,
     * position, handler} method throws a {@link NullPointerException} when the {@code channel}
    ...
    void _ThrowNullPointerException_ChannelIsNull() {
        ...
        // WHEN/THEN
        assertThrows(NullPointerException.class, () -> service.write(channel, position, handler));
    }

    /**
     * Asserts
     * {@link AsynchronousHelloWorld#write(AsynchronousFileChannel, long, CompletionHandler)
     * write(channel, position, handler)} method throws a {@link IllegalArgumentException} when the
    ...
    void _ThrowNullPointerException_ExecutorIsNull() {
        ...
        var channel = mock(AsynchronousFileChannel.class);
        var position = current().nextLong() & MAX_VALUE;
        CompletionHandler<Integer, AsynchronousFileChannel> handler = null;
        // WHEN/THEN
        assertThrows(NullPointerException.class, () -> service.write(channel, position, handler));
    }
}
