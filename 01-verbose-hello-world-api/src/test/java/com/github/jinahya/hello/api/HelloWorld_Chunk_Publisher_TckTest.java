 ...
 * {@link java.util.concurrent.Flow Flow}-flavoured) against {@link HelloWorldChunkPublisher}
 * <em>naively</em> — {@link #createFlowPublisher(long)} returns a fresh publisher with no
 * length-coercing wrapper.
 * <p>
 * {@link HelloWorldChunkPublisher} is open-ended (it never naturally completes), so
 ...
 * @see HelloWorldChunkPublisher
 * @see FlowPublisherVerification
 */
@DisplayName("HelloWorld / chunk Publisher / TCK")
class HelloWorld_Chunk_Publisher_TckTest extends FlowPublisherVerification<byte[]> {

    HelloWorld_Chunk_Publisher_TckTest() {
     ...
     * {@inheritDoc}
     *
     * @param elements ignored — {@link HelloWorldChunkPublisher} is open-ended.
     * @return a fresh {@link HelloWorldChunkPublisher} backed by a fresh
    ...
    public Flow.Publisher<byte[]> createFlowPublisher(final long elements) {
        ...
        return new HelloWorldChunkPublisher(service);
    }
     ...
     * {@inheritDoc}
     *
     * @return {@link Long#MAX_VALUE} — {@link HelloWorldChunkPublisher} never naturally completes.
     */
    @Override
