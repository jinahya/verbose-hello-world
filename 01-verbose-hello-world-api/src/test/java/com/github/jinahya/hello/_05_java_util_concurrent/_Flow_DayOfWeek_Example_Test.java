...
class _Flow_DayOfWeek_Example_Test {
    ...
    private static final class DayOfWeekSubscription implements Flow.Subscription {
    ...
    private static final class DayOfWeekPublisher implements Flow.Publisher<DayOfWeek> {
    ...
    @Test
    void __RequestRandom() {
        final var reference = new AtomicReference<Flow.Subscription>();
        final var subscriber = new DayOfWeekSubscriber(reference) {
            @Override
            public void onSubscribe(final Flow.Subscription subscription) {
