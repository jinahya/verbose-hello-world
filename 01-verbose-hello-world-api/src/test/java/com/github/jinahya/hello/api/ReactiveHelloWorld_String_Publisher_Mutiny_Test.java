...
@Slf4j
class ReactiveHelloWorld_String_Publisher_Mutiny_Test
        extends HelloWorld__FlowPublisher__Test<Flow.Publisher<String>, String> {

    ReactiveHelloWorld_String_Publisher_Mutiny_Test() {
    ...
    class Uni_Test {
        ...
        @Test
        @DisplayName("toUni() → first string (\"hello, world\")")
        void __toUni_firstString() {
            final var s = Multi.createFrom().publisher(publisher())
                    .toUni()
                    .await().atMost(Duration.ofSeconds(10L));
            Assertions.assertEquals(HelloWorldTestUtils.hello_world_string(), s);
        }
    }
