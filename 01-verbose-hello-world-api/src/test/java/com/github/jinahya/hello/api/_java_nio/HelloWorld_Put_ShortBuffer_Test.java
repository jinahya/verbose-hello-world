...
/**
 * A class for testing {@link HelloWorld#put(ShortBuffer) put(buffer)} method.
 *
 * @author Jin Kwon &lt;onacit_at_gmail.com&gt;
 */
@DisplayName("put(ShortBuffer)")
@NoArgsConstructor(access = AccessLevel.PACKAGE)
@Slf4j
@SuppressWarnings({"java:S101"})
class HelloWorld_Put_ShortBuffer_Test
    ...
    void _ThrowNullPointerException_BufferIsNull() {
        ...
        final var buffer = (ShortBuffer) null;
        // ----------------------------------------------------------------------------- when / then
    ...
    void _ThrowBufferOverflowException_BufferRemainingIsLessThan12() {
        // ----------------------------------------------------------------------------------- given
        final var service = service();
        final var buffer = ShortBuffer.allocate(
...
//        final var service = HelloWorldTestUtils.put_buffer_will_increase_buffer_position_by_12(
        ...
        final var buffer = Mockito.spy(ShortBuffer.allocate(HelloWorld.BYTES));
        // ------------------------------------------------------------------------------------ when
    ...
    void _ShouldPut12Ints_(final ByteOrder byteOrder)
        ...
        final var byteBuffer = ByteBuffer.allocate(HelloWorld.BYTES * Short.BYTES);
        final var buffer = Mockito.spy(byteBuffer.order(byteOrder).asShortBuffer());
        // ------------------------------------------------------------------------------------ when
        final var result = service.put(buffer);
        // ------------------------------------------------------------------------------------ then
        final var b = HelloWorldTestUtils.put_buffer12_invoked_once(service);
        ...
        final byte[] fourBytes = new byte[Short.BYTES];
        for (int i = 0; i < buffer.capacity(); i++) {
            byteBuffer.get(i * Short.BYTES, fourBytes);
