...
class HelloWorld_Put_ZipOutputStream_Test
    ...
    void __() throws IOException {
        ...
        // prepare: a mock object of <ZipOutputStream>
        final var stream = Mockito.mock(ZipOutputStream.class);
        final var name = "hello.txt";
        // ------------------------------------------------------------------------------------ when
        try (var construction = Mockito.mockConstruction(ZipEntry.class, (_, context) -> {
            // verify: <new ZipEntry(name)> constructed with the <name>
            Assertions.assertEquals(1, context.arguments().size());
            Assertions.assertEquals(name, context.arguments().getFirst());
        })) {
            final var result = service.put(stream, name);
            // -------------------------------------------------------------------------------- then
            // verify: exactly one <ZipEntry> constructed
            Assertions.assertEquals(1, construction.constructed().size());
            final var entry = construction.constructed().getFirst();
            // verify: <stream.putNextEntry(entry)> invoked, once
            Mockito.verify(stream, Mockito.times(1)).putNextEntry(entry);
            // verify: <stream.closeEntry()> invoked, once
    ...
    @Test
    void _添足_畵蛇_FileOutputStream(@TempDir final File dir) throws IOException {
        // ----------------------------------------------------------------------------------- given
        final var service = service();
        // stub: <service.write(stream)> will write the <hello, world> bytes
        Mockito.doAnswer(i -> {
