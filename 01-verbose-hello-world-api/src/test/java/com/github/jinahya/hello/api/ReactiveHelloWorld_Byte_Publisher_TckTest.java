 ...
 * {@link ReactiveHelloWorldBytePublisher} <em>naively</em> — {@link #createPublisher(long)} returns
 * a fresh publisher with no length-coercing wrapper.
 * <p>
 * {@link ReactiveHelloWorldBytePublisher} always emits exactly {@value HelloWorld#BYTES}
 * {@link Byte} elements followed by {@code onComplete} regardless of the {@code elements} argument,
 * so TCK tests that demand {@code createPublisher(N)} produce exactly {@code N} elements (for
 ...
 */
@DisplayName("ReactiveHelloWorld / byte Publisher / TCK")
class ReactiveHelloWorld_Byte_Publisher_TckTest extends PublisherVerification<Byte> {

    ReactiveHelloWorld_Byte_Publisher_TckTest() {
     ...
     * {@inheritDoc}
     *
     * @param elements ignored — {@link ReactiveHelloWorldBytePublisher} always emits
     *                 {@value HelloWorld#BYTES} elements.
     * @return a fresh {@link ReactiveHelloWorldBytePublisher} backed by a fresh
