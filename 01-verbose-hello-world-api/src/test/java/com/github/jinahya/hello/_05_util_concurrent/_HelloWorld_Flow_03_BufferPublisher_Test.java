...
class _HelloWorld_Flow_03_BufferPublisher_Test extends __HelloWorld_Flow_Test {
    ...
    static class BufferPublisher implements Flow.Publisher<ByteBuffer> { // @formatter:off
        BufferPublisher(final HelloWorld service) {
            super();
    ...
    void __() {
        ...
        final var publisher = Mockito.spy(new BufferPublisher(service));
        final var n = ThreadLocalRandom.current().nextInt(HelloWorld.BYTES >> 1) + 1;
        final var subscriber = Mockito.spy(new Flow.Subscriber<ByteBuffer>() { // @formatter:off
            @Override public String toString() {
                return String.format("[buffer-subscriber@%08x]", hashCode());
            }
            @Override public void onSubscribe(final Flow.Subscription subscription) {
                log.debug("{}.onSubscribe({})", this, subscription);
