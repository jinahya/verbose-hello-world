...
/**
 * A class for testing {@link HelloWorld#put(IntBuffer) put(buffer)} method.
 *
 * @author Jin Kwon &lt;onacit_at_gmail.com&gt;
 */
@DisplayName("put(IntBuffer)")
@NoArgsConstructor(access = AccessLevel.PACKAGE)
@Slf4j
@SuppressWarnings({"java:S101"})
class HelloWorld_Put_IntBuffer_Test
    ...
    void _ThrowNullPointerException_BufferIsNull() {
        ...
        final var buffer = (IntBuffer) null;
        // ----------------------------------------------------------------------------- when / then
    ...
    void _ThrowBufferOverflowException_BufferRemainingIsLessThan12() {
        // ----------------------------------------------------------------------------------- given
        final var service = service();
        final var buffer = IntBuffer.allocate(
    ...
    void _ShouldPut12Ints_(final ByteOrder byteOrder) throws IOException {
        ...
        final var byteBuffer = ByteBuffer.allocate(HelloWorld.BYTES * Integer.BYTES);
        final var buffer = Mockito.spy(byteBuffer.order(byteOrder).asIntBuffer());
        // ------------------------------------------------------------------------------------ when
        ...
        final byte[] fourBytes = new byte[Integer.BYTES];
        for (int i = 0; i < buffer.capacity(); i++) {
            byteBuffer.get(i * Integer.BYTES, fourBytes);
