...
public class JavaBeansUtils2 {
    ...
    private static <H extends PropertyInfoHolder, T> void acceptEachProperty(
        ...
        for (var descriptor : Introspector.getBeanInfo(clazz).getPropertyDescriptors()) {
            ...
            final var child = function.apply(parent).apply(childInfo);
            if (value == null || child == null || !checkHelper(type, value)) {
                continue;
            }
            ...
            if (value instanceof Iterable<?> i) {
                ...
                for (final Object v : i) {
                    final var t = Optional.ofNullable(v).map(Object::getClass).orElse(null);
                    ...
                    final var grandchild = function.apply(child).apply(grandchildInfo);
                    if (v == null || grandchild == null || !check(value)) {
                        continue;
                    }
                    acceptEachProperty(grandchild, v, function);
                    index++;
                }
                continue;
            }
            ...
            acceptEachProperty(child, childInfo.value, function);
        }
    }
