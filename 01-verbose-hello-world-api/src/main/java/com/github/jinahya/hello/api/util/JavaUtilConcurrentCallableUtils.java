 ...
 * checked exceptions — either by passing the exception to a caller-supplied {@link Consumer}, or
 * by wrapping it (as {@link UncheckedIOException} for {@link IOException}, otherwise
...
public class JavaUtilConcurrentCallableUtils {
     ...
     * {@link Exception} so the caller does not have to declare it:
     * {@link IOException} → {@link UncheckedIOException}, anything else → a
     * {@link RuntimeException} whose {@code cause} is the original exception.
     *
     * @param callable the callable to invoke; must not be {@code null}.
     ...
     * @throws RuntimeException     if {@code callable} threw any other {@link Exception}; the
